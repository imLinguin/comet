...
class TokenManager():
    ...
    def obtain_token_for(self, client_id, client_secret):
        ...
        response = self.session.get(f"https://auth.gog.com/token?client_id={client_id}&client_secret={client_secret}&grant_type=refresh_token&refresh_token={self.refresh_token}&without_new_session=1")
    ...
    def get_user_stats(self, user_id):
        ...
        response = self.session.get(f"https://gameplay.gog.com/clients/{self.client_id}/users/{user_id}/stats", headers={"Authorization":f"Bearer {token['access_token']}"})
    ...
    def get_user_achievements(self, user_id):
        ...
        response = self.session.get(f"https://gameplay.gog.com/clients/{self.client_id}/users/{user_id}/achievements", headers={'Authorization': f'Bearer {token["access_token"]}'})
    ...
    def set_user_achievement(self, id, time):
        token = self.tokens.get(self.client_id)
        if not token:
            print("Error, user achievement unlock requested before token")
            return
        payload = {
            "date_unlocked": datetime.datetime.fromtimestamp(time/1000,tz=LOCAL_TIMEZONE).astimezone(datetime.timezone.utc).isoformat(timespec="seconds")
        }

        response = self.session.post(f"https://gameplay.gog.com/clients/{self.client_id}/users/{self.user_id}/achievements/{id}", json=data, headers={'Authorization':f'Bearer {token["access_token"]}'})
